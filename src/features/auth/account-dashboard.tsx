...
const empty: AccountProfile = { name: '', block: '', participantGroup: 'young-adult', languages: ['english'], bio: '' };
export function AccountDashboard() {
  ...
  useEffect(() => {
    ...
    async function load() {
      try {
        ...
        clearSearchDraft();
        const [saved, list] = await Promise.all([accountRequest('/api/account', z.object({ profile: profileSchema.nullable() })), accountRequest('/api/account/interests', listSchema)]);
        if (!active) return;
        setProfile(saved.profile || empty); setItems(list.interests); setReady(true);
  ...
  async function save(event: React.FormEvent<HTMLFormElement>) {
    ...
    try { await accountRequest('/api/account', z.object({ profile: profileSchema }), 'PUT', profile); setMessage('Your profile is saved. We’ll fill in your details next time you find a kaki.'); }
  ...
  return <>
    ...
    {ready && <div className="mt-7 grid items-start gap-7 lg:grid-cols-2">
      <section className="rounded-3xl border border-line bg-paper p-6"><h2 className="text-xl font-semibold">Your resident profile</h2><p className="mt-2 text-sm leading-6 text-muted">Private to your account for now. Your age group doesn’t decide whether you teach or learn.</p>
        <form onSubmit={save} className="mt-5 space-y-4">
          ...
          <div><label className="field-label" htmlFor="profile-bio">A little about you (optional)</label><textarea id="profile-bio" className="field min-h-24" maxLength={600} value={profile.bio} onChange={e => setProfile({ ...profile, bio: e.target.value })} placeholder="What do you enjoy sharing or learning?" /></div>
      ...
      <section aria-labelledby="my-interests"><div className="flex flex-wrap items-center justify-between gap-3"><h2 id="my-interests" className="text-xl font-semibold">My interests</h2><Link href="/#find-kaki" className="min-h-11 content-center text-sm font-semibold text-kampung-red">Find another kaki →</Link></div>
        <p className="mt-2 text-sm leading-6 text-muted">Your latest 100 selections made while signed in. Guest selections stay separate. These are demo connections, not messages or bookings.</p>
        {!items.length && <div className="mt-5 rounded-3xl border border-line bg-paper p-6"><p className="font-semibold">Your first connection is waiting.</p><p className="mt-2 text-sm leading-6 text-muted">Find a kaki and express interest to see them here.</p></div>}
