...
import { SendRequestButton } from '@/features/connections/send-request-button';
...
export function ResultsClient() {
  ...
  return <>
    <div className="flex flex-wrap items-start justify-between gap-6">
      <div><p className="text-xs font-bold uppercase tracking-[0.17em] text-kampung-red">A neighbour today. A kaki tomorrow.</p><h1 className="mt-4 text-3xl font-semibold tracking-tight sm:text-4xl">Meet your potential kakis.</h1><p className="mt-4 max-w-2xl text-base leading-7 text-muted">A shared activity is a lovely place to start. Here’s who could join you.</p></div>
    ...
    <p className="mt-6 text-xs leading-6 text-muted">Community neighbours opt in to matching. Fictional profiles are labelled “Demo profile”. CC facilities and times remain illustrative · Singapore time</p>
    {result.matches.length === 0 ? <div className="mt-8 max-w-2xl rounded-3xl border border-line bg-paper p-8"><h2 className="text-2xl font-semibold">Your kaki may be just around the corner.</h2><p className="mt-4 leading-7 text-muted">We don’t have a compatible neighbour in the community directory yet. Try a broader skill, another activity, or a different language or generation preference.</p><Link href="/#find-kaki" className="primary-button mt-6">Adjust my request</Link></div> : <>
      ...
      <div className="mt-5 grid items-stretch gap-6 md:grid-cols-2 xl:grid-cols-3">
        {result.matches.map(match => <article key={`${session.id}:${match.resident.id}`} className="flex min-w-0 flex-col rounded-[1.7rem] border border-line bg-paper p-6 sm:p-7">
          <div className="flex items-center gap-4"><div className="flex h-14 w-14 shrink-0 items-center justify-center rounded-2xl bg-avatar text-lg font-semibold text-avatar-ink" aria-hidden="true">{match.resident.name.split(/[\s&]+/).filter(Boolean).slice(0, 2).map(part => part[0]).join('')}</div><div className="min-w-0"><h2 className="text-xl font-semibold leading-7">{match.resident.name}</h2><p className="mt-1 text-sm text-muted">{match.resident.ageRange} · Blk {match.resident.block}</p></div></div>
          <p className="mt-5 text-sm font-semibold text-kampung-red">{match.reasons[0]}</p>
          <p className="mt-2 text-xs font-semibold text-muted">{match.resident.isDemo === false ? 'Community neighbour' : 'Demo profile'}</p>
          <p className="mt-3 text-sm leading-7 text-muted">{match.resident.bio}</p>
          <p className="mt-3 text-xs leading-6 text-muted">{match.resident.isDemo === false ? 'Languages: ' : 'Cambridge Road · '}{match.resident.languages.map(language => LANGUAGE_LABELS[language]).join(' / ')}</p>
          <div className="mt-5 rounded-2xl border border-line bg-cream p-4"><div className="flex flex-wrap items-center justify-between gap-2"><span className="text-xs font-semibold text-bridge-ink">{BRIDGE_LABELS[session.request.participantGroup]} ↔ {BRIDGE_LABELS[match.resident.participantGroup]}</span><span className="text-sm font-bold text-kampung-red">{match.bridgeScore}<span className="font-normal text-muted">/100</span></span></div><p className="mt-1 text-xs text-muted">Generation bridge score</p></div>
          ...
          {match.resident.isDemo === false
            ? <SendRequestButton key={`${session.id}:${result.ownerId || 'guest'}`} match={match} session={session} ownerId={result.ownerId} />
            : <InterestButton key={`${session.id}:${result.ownerId || 'guest'}`} match={match} session={session} storageMode={result.storageMode} ownerId={result.ownerId} />}
        </article>)}
      </div>
    </>}
